...
const jwtSecret =
  "576df66cea407b30c66f3e8d87b52ac8222d0939bff76bdca8825f4d45361dbe31eaad";
...
exports.communityAuth = (req, res, next) => {
  const token = req.cookies.jwt;
  if (token) {
    jwt.verify(token, jwtSecret, (err, decodedToken) => {
      if (err) {
        ...
        if (decodedToken.role !== "community") {
          return res.status(401).json({ message: "Not authorized" });
        } else {
          next();
        }
...
exports.startupAuth = (req, res, next) => {
  const token = req.cookies.jwt;
  if (token) {
    jwt.verify(token, jwtSecret, (err, decodedToken) => {
      if (err) {
